...
import { 
  Book, Search, Play, Clock, LayoutGrid, List, 
  Sparkles, Loader2, Heart, Beaker 
} from 'lucide-react';
              ...
              <div className="flex flex-wrap justify-center gap-2 md:gap-3">
                {['Anxious', 'Grateful', 'Lost', 'Weak', 'Sad'].map((mood) => (
                  <button 
                    ...
                    className="px-5 md:px-8 py-3 md:py-4 rounded-2xl bg-[#fdfcf8] border border-gray-100 text-[10px] font-black uppercase tracking-widest hover:bg-[#D4AF37] hover:text-white transition-all shadow-sm active:scale-95 text-gray-800"
                  >
                    {mood}
                  </button>
                ))}
                  ...
                  {activeTab === 'surah' && filteredSurahs.map((surah: any) => (
                    ...
                    >
                      <div className="flex items-center gap-4 md:gap-6 relative z-10 min-w-0 flex-1">
                        <div className="w-12 h-12 md:w-14 md:h-14 bg-[#fdfcf8] group-hover:bg-[#1a2e2a] group-hover:text-[#D4AF37] rounded-2xl flex items-center justify-center text-xs font-black transition-all shrink-0 text-gray-800">
                          {surah.number}
                        </div>
                    ...
                    <div className="col-span-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                      {Array.from({ length: 30 }, (_, i) => (
                        <motion.div 
                          ...
                          className="bg-white border border-gray-100 p-8 rounded-[2rem] text-center cursor-pointer transition-all shadow-sm group text-gray-800"
                        >
